main <- function(n){
  ...
  ##################################################################
    ...
    if(length(Metric21) >= Na && Na > 0){
      ...
      weight_to_center1<-((Metric21[order(Metric21)][1:Na]*-1)+Metric21[order(Metric21)][Na])/sum((Metric21[order(Metric21)][1:Na]*-1)+Metric21[order(Metric21)][Na])
    }else{
      weight_to_center1<-((Metric21[order(Metric21)]*-1)+Metric21[which.max(Metric21)])/sum((Metric21[order(Metric21)]*-1)+Metric21[which.max(Metric21)])
      # Na <- length(Metric2)
    }
      
      analogs.final.local1[is.na(analogs.final.local1)]<-mean(analogs.final.local1, na.rm = TRUE)
      analogs.final.local1 <- analogs.final.local1*weight_to_center1
      result1 = sum(analogs.final.local1, na.rm = TRUE)
  ...
  if (sum(is.na(y2)) < .5*(2*M+1)) {
    ...
    if(length(Metric22) >= Na && Na > 0){
      ...
      weight_to_center2<-((Metric22[order(Metric22)][1:Na]*-1)+Metric22[order(Metric22)][Na])/sum((Metric22[order(Metric22)][1:Na]*-1)+Metric22[order(Metric22)][Na])
    }else{
      weight_to_center2<-((Metric22[order(Metric22)]*-1)+Metric22[which.max(Metric22)])/sum((Metric22[order(Metric22)]*-1)+Metric22[which.max(Metric22)])
      # Na <- length(Metric2)
    }
    
    analogs.final.local2[is.na(analogs.final.local2)]<-mean(analogs.final.local2, na.rm = TRUE)
    analogs.final.local2 <- analogs.final.local2*weight_to_center2
    result2 = sum(analogs.final.local2, na.rm = TRUE)
