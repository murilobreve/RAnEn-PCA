...
pred1$stamp <-
  as.POSIXct(pred1$t * 60, origin = pred1$epoch, tz = "UTC") # To 30 min resolution
...
if (nb.stations >= 2) {
  ...
  pred2$stamp <-
    as.POSIXct(pred2$t * 60, origin = pred2$epoch, tz = "UTC") # To 30 min resolution
...
if (nb.stations >= 3) {
  ...
  pred3$stamp <-
    as.POSIXct(pred3$t * 60, origin = pred3$epoch, tz = "UTC") # To 30 min resolution
  ...
  pred3$t0 <-
    (as.numeric(as.POSIXct(forecasting.t0, tz = "UTC")) - as.numeric(pred3$epoch)) / 60
  pred3$n0 <-
...
target$stamp <-
  as.POSIXct(target$t * 60, origin = target$epoch, tz = "UTC") # To 30 min resolution
