...
setwd("~/")
setwd("AnEnMDataR/whether-station-data")
...
pred1$stamp <-
  as.POSIXct(pred1$t * 60 * 30, origin = pred1$epoch, tz = "UTC") # To 30 min resolution

pred1$WSPD <- ncvar_get(ds, "WSPD")
if (ncatt_get(ds, "WSPD", "_FillValue")$hasatt) {
  pred1$WSPD[pred1$WSPD == ncatt_get(ds, "WSPD", "_FillValue")$value] <-
    NA
} else {
  pred1$WSPD[pred1$WSPD > 9e+36] <- NA  # missing value
}

pred1$ATMP <- ncvar_get(ds, "ATMP")
if (ncatt_get(ds, "ATMP", "_FillValue")$hasatt) {
  pred1$ATMP[pred1$ATMP == ncatt_get(ds, "ATMP", "_FillValue")$value] <-
    NA
} else {
  pred1$ATMP[pred1$ATMP > 9e+36] <- NA  # missing value
}

pred1$WDIR <- ncvar_get(ds, "WDIR")
if (ncatt_get(ds, "WDIR", "_FillValue")$hasatt) {
  pred1$WDIR[pred1$WDIR == ncatt_get(ds, "WDIR", "_FillValue")$value] <- NA
} else {
  pred1$WDIR[pred1$WDIR > 9e+36] <- NA  # missing value
}

pred1$PRES <- ncvar_get(ds, "PRES")
if (ncatt_get(ds, "PRES", "_FillValue")$hasatt) {
  pred1$PRES[pred1$PRES == ncatt_get(ds, "PRES", "_FillValue")$value] <-
    NA
} else {
  pred1$PRES[pred1$PRES > 9e+36] <- NA  # missing value
}
pred1$GST <- ncvar_get(ds, "GST")


if (ncatt_get(ds, "GST", "_FillValue")$hasatt) {
  pred1$GST[pred1$GST == ncatt_get(ds, "GST", "_FillValue")$value] <-
    NA
} else {
  pred1$GST[pred1$GST > 9e+36] <- NA  # missing value
}
nc_close(ds)
# 1.1 - Interpolation between start_of_date and end_of_date

pred1.interp <-
  .InterpolateData(pred1$stamp, pred1$WSPD, start_of_date, end_of_date)
pred1$WSPD <- pred1.interp[, 3]
pred1.interp <-
  .InterpolateData(pred1$stamp, pred1$ATMP, start_of_date, end_of_date)
pred1$ATMP <- pred1.interp[, 3]
pred1.interp <-
  .InterpolateData(pred1$stamp, pred1$PRES, start_of_date, end_of_date)
pred1$PRES <- pred1.interp[, 3]
pred1.interp <-
  .InterpolateData(pred1$stamp, pred1$GST, start_of_date, end_of_date)
pred1$GST <- pred1.interp[, 3]
pred1.interp <-
  .InterpolateData(pred1$stamp, pred1$WDIR, start_of_date, end_of_date)
pred1$WDIR <- pred1.interp[, 3]
pred1$stamp <- pred1.interp[, 1]

# 1.12 - Transforming to 22.5 degree resolution, 16 different cardinales (Ex: NNE, NNW)

pred1$WDIR<- 22.5*(round(pred1$WDIR/ 22.5))

# 1.2 - Setting the intersection between forecasting.t0 and the pred1$stamp
pred1$t0 <-
  (as.numeric(as.POSIXct(forecasting.t0, tz = "UTC")) - as.numeric(pred1$epoch)) / 60 / 30
pred1$t0
...
if (nb.stations >= 2) {
  ...
  pred2$stamp <-
    as.POSIXct(pred2$t * 60 * 30, origin = pred2$epoch, tz = "UTC") # To 30 min resolution
  pred2$WSPD <- ncvar_get(ds, "WSPD")
  if (ncatt_get(ds, "WSPD", "_FillValue")$hasatt) {
    pred2$WSPD[pred2$WSPD == ncatt_get(ds, "WSPD", "_FillValue")$value] <-
      NA
  } else {
    pred2$WSPD[pred2$WSPD > 9e+36] <- NA  # missing value
  }
  pred2$WDIR <- ncvar_get(ds, "WDIR")
  if (ncatt_get(ds, "WDIR", "_FillValue")$hasatt) {
    pred2$WDIR[pred2$WDIR == ncatt_get(ds, "WDIR", "_FillValue")$value] <- NA
  } else {
    pred2$WDIR[pred2$WDIR > 9e+36] <- NA  # missing value
  }
  pred2$ATMP <- ncvar_get(ds, "ATMP")
  if (ncatt_get(ds, "ATMP", "_FillValue")$hasatt) {
    pred2$ATMP[pred2$ATMP == ncatt_get(ds, "ATMP", "_FillValue")$value] <-
      NA
  } else {
    pred2$ATMP[pred2$ATMP > 9e+36] <- NA  # missing value
  }
  pred2$PRES <- ncvar_get(ds, "PRES")
  if (ncatt_get(ds, "PRES", "_FillValue")$hasatt) {
    pred2$PRES[pred2$PRES == ncatt_get(ds, "PRES", "_FillValue")$value] <-
      NA
  } else {
    pred2$PRES[pred2$PRES > 9e+36] <- NA  # missing value
  }
  pred2$GST <- ncvar_get(ds, "GST")
  if (ncatt_get(ds, "GST", "_FillValue")$hasatt) {
    pred2$GST[pred2$GST == ncatt_get(ds, "GST", "_FillValue")$value] <-
      NA
  } else {
    pred2$GST[pred2$GST > 9e+36] <- NA  # missing value
  }
  nc_close(ds)

  # 2.1 - Interpolation between start_of_date and end_of_date
  pred2.interp <-
    .InterpolateData(pred2$stamp, pred2$WSPD, start_of_date, end_of_date)
  pred2$WSPD <- pred2.interp[, 3]
  pred2.interp <-
    .InterpolateData(pred2$stamp, pred2$ATMP, start_of_date, end_of_date)
  pred2$ATMP <- pred2.interp[, 3]
  pred2.interp <-
    .InterpolateData(pred2$stamp, pred2$PRES, start_of_date, end_of_date)
  pred2$PRES <- pred2.interp[, 3]
  pred2.interp <-
    .InterpolateData(pred2$stamp, pred2$GST, start_of_date, end_of_date)
  pred2$GST <- pred2.interp[, 3]
  pred2.interp <-
    .InterpolateData(pred2$stamp, pred2$WDIR, start_of_date, end_of_date)
  pred2$WDIR <- pred2.interp[, 3]
  pred2$stamp <- pred2.interp[, 1]

  pred2$WDIR <- 22.5*(round(pred2$WDIR / 22.5))

  # 2.2 - Setting the intersection between forecasting.t0 and the pred2$stamp
  pred2$t0 <-
    (as.numeric(as.POSIXct(forecasting.t0, tz = "UTC")) - as.numeric(pred2$epoch)) / 60
  pred2$t0
  pred2$n0 <-
...
target$stamp <-
  as.POSIXct(target$t * 60 * 30, origin = target$epoch, tz = "UTC") # To 30 min resolution
target$WSPD <- ncvar_get(ds, "WSPD")
if (ncatt_get(ds, "WSPD", "_FillValue")$hasatt) {
  target$WSPD[target$WSPD == ncatt_get(ds, "WSPD", "_FillValue")$value] <-
    NA
} else {
  target$WSPD[target$WSPD > 9e+36] <- NA  # missing value
}
target$ATMP <- ncvar_get(ds, "ATMP")
if (ncatt_get(ds, "ATMP", "_FillValue")$hasatt) {
  target$ATMP[target$ATMP == ncatt_get(ds, "ATMP", "_FillValue")$value] <-
    NA
} else {
  target$ATMP[target$ATMP > 9e+36] <- NA  # missing value
}
target$PRES <- ncvar_get(ds, "PRES")
if (ncatt_get(ds, "PRES", "_FillValue")$hasatt) {
  target$PRES[target$PRES == ncatt_get(ds, "PRES", "_FillValue")$value] <-
    NA
} else {
  target$PRES[target$PRES > 9e+36] <- NA  # missing value
}
target$GST <- ncvar_get(ds, "GST")
if (ncatt_get(ds, "GST", "_FillValue")$hasatt) {
  target$GST[target$GST == ncatt_get(ds, "GST", "_FillValue")$value] <-
    NA
} else {
  target$GST[target$GST > 9e+36] <- NA  # missing value
}

target$WDIR <- ncvar_get(ds, "WDIR")
if (ncatt_get(ds, "WDIR", "_FillValue")$hasatt) {
  target$WDIR[target$WDIR == ncatt_get(ds, "WDIR", "_FillValue")$value] <- NA
} else {
  target$WDIR[target$WDIR > 9e+36] <- NA  # missing value
}
nc_close(ds)

# 4.1 - Interpolation between start_of_date and end_of_date
target.interp <-
  .InterpolateData(target$stamp, target$WSPD, start_of_date, end_of_date)
target$WSPD <- target.interp[, 3]
target.interp <-
  .InterpolateData(target$stamp, target$ATMP, start_of_date, end_of_date)
target$ATMP <- target.interp[, 3]
target.interp <-
  .InterpolateData(target$stamp, target$PRES, start_of_date, end_of_date)
target$PRES <- target.interp[, 3]
target.interp <-
  .InterpolateData(target$stamp, target$GST, start_of_date, end_of_date)
target$GST <- target.interp[, 3]
target.interp <-
  .InterpolateData(target$stamp, target$WDIR, start_of_date, end_of_date)
target$WDIR <- target.interp[, 3]
target$stamp <- target.interp[, 1]
target$WDIR <- 22.5 * (round(target$WDIR/ 22.5))

# 4.2 - Setting the intersection between forecasting.t0 and the target$stamp
target$t0 <-
  (as.numeric(as.POSIXct(forecasting.t0, tz = "UTC")) - as.numeric(target$epoch)) / 60
target$n0 <-
...
if (wdir == TRUE) {
  data.pca <- data.frame(pred1$WSPD, pred1$ATMP, pred1$PRES,
                         pred1$GST, pred1$WDIR)
  colnames(data.pca) <-
    c("WSPD", "ATMP", "PRES", "GST", "WDIR")
  if (nb.stations > 1) {
    data.pca2 <- data.frame(pred2$WSPD, pred2$ATMP, pred2$PRES,
                            pred2$GST, pred2$WDIR)
    colnames(data.pca2) <-
      c("WSPD", "ATMP", "PRES", "GST", "WDIR")
  }
} else {
  data.pca <- data.frame(pred1$WSPD, pred1$ATMP, pred1$PRES,
                         pred1$GST)
  colnames(data.pca) <- c("WSPD", "ATMP", "PRES", "GST")
  if (nb.stations > 1) {
    data.pca2 <- data.frame(pred2$WSPD, pred2$ATMP, pred2$PRES,
                            pred2$GST)
    colnames(data.pca2) <-
      c("WSPD", "ATMP", "PRES", "GST")
  }
}

data.1 <- data.frame(pred1$WSPD, pred1$GST, pred1$WDIR)
data.2 <- data.frame(pred2$WSPD, pred2$GST, pred2$WDIR)

data.merged<-cbind(data.pca,data.pca2)
colnames(data.merged) <-
  c("Soutelo WSPD","Soutelo  GST","Soutelo  WDIR","Edroso WSPD", "Edroso GST", "Edroso WDIR")

colnames(data.merged) <-
  c("Soutelo WSPD", "Soutelo ATMP", "Soutelo PRES", "Soutelo GST", "Soutelo WDIR","Edroso WSPD", "Edroso ATMP", "Edroso PRES", "Edroso GST", "Edroso WDIR")

#data.pca.scaled <- scale(data.pca)
data.pca.scaled.na <- na.omit(data.pca)

pca <- prcomp(na.omit(data.merged), center = TRUE, scale. = TRUE)
pc1 <- pca$x[, 1]
pc2 <- pca$x[, 2]
pc3 <- pca$x[, 3]

write.csv(pca$rotation,file = "wind related.csv")
...
data.pca.1_3[complete.cases(data.merged), 1] <- pc1
data.pca.1_3[complete.cases(data.merged), 2] <- pc2
data.pca.1_3[complete.cases(data.merged), 3] <- pc3

pred1[[v.1]] <- data.pca.1_3[, 1]

if (nb.stations > 1) {
  pca.2 <- prcomp(na.omit(data.pca2), center = TRUE, scale. = TRUE)
  pc1.2 <- pca.2$x[, 1]
  pc2.2 <- pca.2$x[, 2]
  pc3.2 <- pca.2$x[, 3]
  ...
  data.pca2.1_3 <-
    matrix(data = NA,
           nrow = length(data.pca2[, 1]),
           ncol = 3)

  data.pca2.1_3[complete.cases(data.pca2), 1] <- pc1.2
  data.pca2.1_3[complete.cases(data.pca2), 2] <- pc2.2
  data.pca2.1_3[complete.cases(data.pca2), 3] <- pc3.2
  pred2[[v.1]] <- data.pca2.1_3[, 1]
}

if (nb.pca >= 2) {
  pred2 <- list()
  pred2[[v.1]] <- data.pca.1_3[, 2]
  pred2$stamp <- pred1$stamp
  pred2$t <- pred1$t
  pred2$t0 <- pred1$t0
  pred2$N <- pred1$N
  pred2$n0 <- pred1$n0
...
if (nb.pca >= 3) {
  pred3 <- list()
  pred3[[v.1]] <- data.pca.1_3[, 3]
  pred3$stamp <- pred1$stamp
  pred3$t <- pred1$t
  pred3$t0 <- pred1$t0
  pred3$N <- pred1$N
  pred3$n0 <- pred1$n0
...
message("[INFO] #Stations: ", nb.stations)
if(nb.stations < 2){
  nb.stations <- nb.pca
  nb.series <- nb.stations
}
...
validHours <-
  (format(strptime(pred1$stamp, "%Y-%m-%d %H:%M:%S"), '%H:%M:%S') >=  format(
    paste(startH, ':', startM, ':', startS),
    format = "%H:%M:%S",
    tz = "UTC"
  )) &
  (format(strptime(pred1$stamp, "%Y-%m-%d %H:%M:%S"), '%H:%M:%S') <=  format(
    paste(endH, ':', endM, ':', endS),
    format = "%H:%M:%S",
    tz = "UTC"
  ))
