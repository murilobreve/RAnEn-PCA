...
main <- function(n) {
    ...
    if (sum(is.na(y2)) < .5 * (2 * M + 1)) {              # guarantees window has 50% of valid values
    ...
    if (nb.stations > 2) {
        ...
        if (sum(is.na(y3)) < .5 * (2 * M + 1)) {            # guarantees window has 50% of valid values
            ...
            if(v.aim == "WDIR")
                result3 <- mean.circular(analogs.final3, na.rm = TRUE)
    ...
    if (length(na.omit(c(result1, result2, result3))) == 0) {
        ...
        result.final <- sum(result.na) / (length(result.na))
        if(v.aim == "WDIR")
            result.final <- mean.circular(result.na)
            if(result.final < 0)
                result.final <- result.final + 2*pi
    }
